...
// 	}
		...
		script {
		   def latestTag = sh(returnStdout: true, script: 'git describe --abbrev=0 --tags || echo "0.0.0"').trim()
		      ...
		      aws ecs register-task-definition --cli-input-json file://task-definition.json --region us-east-1 "${sed "s#.*image.*#  \"image\": \"${REPOSITORY_URI}:${latestTag}\","#" task-definition.json}" > /dev/null
