...
// 		     sh "docker tag jenkinstest:latest ${REPOSITORY_URI}:${latestTag}"
		...
		script {
		   def latestTag = sh(returnStdout: true, script: 'git describe --abbrev=0 --tags || echo "0.0.0"').trim()
			...
			sh "aws ecs register-task-definition --cli-input-json file://task-definition.json --region us-east-1 --family inn-dev-td-0e6cf42e2321:${latestTag} --network-mode bridge --container-definitions '$(jq --arg newImage ${REPOSITORY_URI}:${latestTag} '.containerDefinitions[0].image = $newImage' task-definition.json)'"
//                       sh "aws ecs register-task-definition --cli-input-json file://task-definition.json --region us-east-1 --family inn-dev-td-0e6cf42e2321:${latestTag} --network-mode bridge"
                 }
	       }
            }
        }
